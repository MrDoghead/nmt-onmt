...
def main(in_dir, out_dir, base_name):
    ...
    # 第四步：生成bpe code
    code_path = _main_create_bpe(out_dir, train_arr)

    # 第五步： 对所有的测试数据以及训练数据进行bpe 编码
    _main_apply_bpe(out_dir, train_arr, code_path)
    _main_apply_bpe(out_dir, val_arr, code_path)
...
def _main_create_bpe(out_dir, f_arr):
    ...
    :param 构建
    """
    cur_dir = os.path.dirname(f_arr[0])
    train_tot = os.path.join(cur_dir, "train_tot")
    with open(train_tot, "w") as out_:
        for f_path in f_arr:
            with open(f_path) as in_:
                for line in in_:
                    out_.write(line)
    code_path = os.path.join(out_dir, "code")
    # 利用bpe构建bpe code file
    with open(train_tot) as in_:
        with open(code_path, "w") as out_:
            learn_bpe.learn_bpe(in_, out_, BPE_TOKENS)
    return code_path


def _main_apply_bpe(out_dir, f_arr, code_file):
    with open(code_file) as f_code:
