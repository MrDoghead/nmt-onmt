...
dest_data="/data/translate/hb_trans/data_bin/fq_1kw_en_zh_v3"
mkdir -p  ${dest_data}
SAVE="/data/translate/hb_trans/model_bin/fq_1kw_en_zh_v3"
mkdir -p $SAVE
...
# for test
MKL_THREADING_LAYER=GNU CUDA_VISIBLE_DEVICES=1,2,3,4,5,6,7 $(which fairseq-train) ${dest_data} --clip-norm 0 --optimizer adam --lr 1e-3 \
--source-lang en --target-lang zh --max-tokens 4096  --log-interval 1000 --min-lr '1e-09' --weight-decay 0.0001 \
...
-a transformer --share-decoder-input-output-embed --save-dir $SAVE \
--dropout 0.3 --attention-dropout 0.1 --update-freq 4
